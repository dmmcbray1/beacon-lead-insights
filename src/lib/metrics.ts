...
import { CONTACT_DISPOSITIONS, QUOTE_DISPOSITIONS, BAD_PHONE_STATUSES } from './constants';
...
export function calculateKPIs(leads: LeadRecord[]): KPIData {
  const totalLeads = leads.length;
  const newLeads = leads.filter(l => l.lead_type === 'new_lead').length;
  const reQuoteLeads = leads.filter(l => l.lead_type === 're_quote').length;

  const contactedLeads = leads.filter(l => isContact(l.statuses));
  ...
  const totalCallbacks = leads.filter(l => l.total_callbacks > 0).length;
  const badPhoneCount = leads.filter(l => l.has_bad_phone).length;

  const contactRate = calcRate(totalContacts, totalLeads);
  const quoteRate = calcRate(totalQuotedHouseholds, totalLeads);

  // Contact to quote: of contacted leads, how many were quoted
